...
namespace WuCore\ResourcesBundle\Services;
...
if (!defined('FILE_CACHE_DIRECTORY'))
    define('FILE_CACHE_DIRECTORY', __DIR__ . '/../Resources/private/images/cache');    // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if (!defined('NOT_FOUND_IMAGE'))
    define('NOT_FOUND_IMAGE', __DIR__ . '/../Resources/private/images/no-img.png');        // Image to serve if any 404 occurs 
if (!defined('ERROR_IMAGE'))
    define('ERROR_IMAGE', __DIR__ . '/../Resources/private/images/no-img.png');         // Image to serve if an error occurs instead of showing error message 
...
class timthumb {
    ...
    protected function handleErrors() {
        if ($this->haveErrors()) {
            ...
            if (ERROR_IMAGE) {
                if ($this->serveInternalImage(ERROR_IMAGE)) {
                    exit(0);
                } else {
    ...
    protected function serveInternalImage($image = null) {
        if ($image) {
            $this->localImage = realpath($image);
    ...
    protected function getLocalImagePath($src) {
        ...
        if ($absolute && file_exists($absolute)) { //realpath does file_exists check, so can probably skip the exists check here
            ...
            $this->docRoot = str_replace('web', '', $this->docRoot);
            if (stripos($absolute, realpath($this->docRoot)) === 0) {
                return $absolute;
            } else {
                $this->debug(1, "Security block: The file specified occurs outside the document root.");
                //and continue search
